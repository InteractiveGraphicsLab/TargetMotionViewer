...
template<typename T>
std::vector<T> subvector(const std::vector<T>& v, int start_idx, int end_idx)
{
  std::vector<T> vec(end_idx - start_idx + 1);
  std::copy(v.begin() + start_idx, v.begin() + end_idx, vec.begin());
  return vec;
}
